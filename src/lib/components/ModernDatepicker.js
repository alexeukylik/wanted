...
class ModernDatepicker extends Component {
    ...
    handleDateChange(value, unit, force) {
        ...
        let dateToEdit = date.format(format || 'DD-MM-YYYY');
        if (force) {
            dateToEdit = value;
            unit = 'year';
        }
    ...
    toggleCalendar(bool, e) {
        ...
        this.setState({
            ...
            inputValue: bool ? (date ? (isInstanceOfDate ? dayjs(date).format(format || 'DD-MM-YYYY') : dayjs(date, format || 'DD-MM-YYYY').format(format || 'DD-MM-YYYY')) : '') : this.state.dateToEdit,
    ...
    editDateInput(e) {
        let target = e.target;
        if (e.keyCode === 13) {
            this.handleDateChange(this.state.dateToEdit, 'year', true);
        }
        if (target.value.length !== 10) {
            this.setState({
                inputValue: e.target.value
            }, () => {
                this.dateInputMask(target);
            });
        } else {
            this.setState({
                inputValue: e.target.value,
                dateToEdit: e.target.value ? e.target.value : ''
            }, ()=>{
                this.handleDateChange(this.state.dateToEdit, 'year', true);
            });
        }
        ...
        return (
            <Container onBlur={(e) => this.onBlur(e)}>
                ...
                <Input
                    className={className}
                    id={id}
                    onChange={(e) => this.editDateInput(e)}
                ...
                />}
                {showContainer && isValid && (this.checkAndReturnDate() === '' || dayjs(this.checkAndReturnDate(), format || 'DD-MM-YYYY').isValid()) &&
                <CalendarContainer secondaryColor={secondaryColor}>
